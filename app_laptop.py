...
lst = ['A', 'B', 'C', 'D', 'Ap', 'Bp', 'Cp', 'Dp']
lstx = [0, 0, 0, 0, 1000, 1000, 1000, 1000]
lsty = [EyeboxFullWidth/2, 
        -EyeboxFullWidth/2, 
        -EyeboxFullWidth/2, 
        EyeboxFullWidth/2, 
        1000*np.arctan(FullHorizontalFOV/2), 
        -1000*np.arctan(FullHorizontalFOV/2), 
        -1000*np.arctan(FullHorizontalFOV/2), 
        1000*np.arctan(FullHorizontalFOV/2)]
lstz = [EyeboxFullHeight/2, 
        EyeboxFullHeight/2, 
        -EyeboxFullHeight/2, 
        -EyeboxFullHeight/2, 
        1000*np.arctan(FullVerticalFOV/2), 
        1000*np.arctan(FullVerticalFOV/2), 
        -1000*np.arctan(FullVerticalFOV/2), 
        -1000*np.arctan(FullVerticalFOV/2)]
lstcolor = ["Eyebox", 
...
df_3d = pd.DataFrame(list(zip(lst, lstx, lsty, lstz, lstcolor)),
               columns =['Name', 'x', 'y', 'z', 'color'])

fig_3d = px.line_3d(df_3d, x="x", y="y", z="z", color="color", range_x=[0,1100], range_y=[-1500,1500], range_z=[-1500,1500])
